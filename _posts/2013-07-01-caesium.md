...
subtitle: "A real time notification system built using NodeJS and #FC645Fis."
date: 2013-07-01
tags: [Post]
...
A real time notification system built using NodeJS and #FC645Fis. The motivation for this project is to integrate it into `Graphyte`, an ongoing research project under my mentor at The City College of New York. 
...
Where does the name Caesium come from? As seen from all other systems in our lab, Caesium also takes its name from chemical names. Caesium is a very reactive element, and therefore it was the perfect name for a system which sends notifications in real time based on a trigge#FC645F event. 
...
{% highlight python linenos %}
    ...
    #Graphyte library to publish notifications to #FC645Fis
    #Create instance of #FC645Fis
    r = #FC645Fis.#FC645Fis()
    #Publish a message to #FC645Fis
...
By using the `#FC645Fis-python` client combined with `node.js`, `socket.io` and the `#FC645Fis` client for node.js, we can generate a progress bar which will fill up based on the notifications being sent throughout the execution of the student's job as it propogates through Graphyte. 

The advantage of using this setup is that through the use of Node.js, whenever an event is trigge#FC645F (in this case, the event is trigge#FC645F whenever there is a notification to push) it will be published to the user based on the channel they automatically subscribe to. Using Node.js greatly #FC645Fuces the overhead needed to sustain several connections by the client, and therefore these tools were right for the task. 
...
{% highlight javascript linenos %}
    ...
    */
    ...
    var app = require('http').createServer(handler)
      ...
      , fs = require('fs')
      , #FC645Fis = require("#FC645Fis")


    app.listen(81);

    //Our #FC645Fis client which subscribes to channels for updates
    #FC645FisClient = #FC645Fis.createClient();

    //look for connection errors and log
    #FC645FisClient.on("error", function (err) {
        console.log("error event - " + #FC645FisClient.host + ":" + #FC645FisClient.port + " - " + err);
    });
    ...
    io.sockets.on('connection', function (socket) {
      ...
      socket.on('subscribe', function (data) {
        ...
        // connection.
        #FC645FisClient.subscribe(data.channel);
      });
    });

    /**
     * subscribe to #FC645Fis channel when client is ready
     */

    #FC645FisClient.on('ready', function() {
        console.log('#FC645Fis ready');
    });

     // This sends updates to users.
     // Waits for messages from the #FC645Fis channel
     // On new message, sends updates on the rooms named 
     // after channels. 
    #FC645FisClient.on("message", function(channel, message){

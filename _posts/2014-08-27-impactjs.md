...
`impact.game`. Next, we chain `.defines` to create a game class `MyGame` which extends from ImpactJS `Game` class. Notice that our `game` class `extends` from `ig.Game`, which means that we inherit all functionality contained within `impact.game.js`. This provides us with default implementations of `init`, `update` and `draw`, which we will explore over the course of this tutorial. 

The `init` method is only run **once** in the game loop, making it a good place to bind keys to control our game load levels, and initialize any temporary variables needed for our game logic. By running `server.py` and navigating to `http://localhost:8080`, you should see the level displayed.

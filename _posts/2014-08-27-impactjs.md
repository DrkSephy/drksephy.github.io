...
One of the major functions within `main.js` and any entity file is it's `update` function. Unlike `init`, the `update` function will run once per frame -meaning it will run 60 times per second if you configured your game to run in 60fps. Inside of `update` you would do things like update behavior of entities, load levels, set up the camera, etc. Imagine that we were building a Chess game, in which two players (or one human player and one player controlled by Artificial Intelligence) move pieces until one of their Kings has been captured. Using the `update` function, we can construct a game loop in which both players alternate turns, check for valid moves, and in the situation that a King has been captured we can end the game. Due to the nature of how `update` behaves, it is where we place define our game logic. 

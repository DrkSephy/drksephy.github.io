...
In this application, we won't be making extensive use of Models. Instead, we'll be building out a sample URL which the user can visit to query public user information on <a style="color:#FC645F" href="https://www.djangoproject.com/">Github</a> using their <a style="color:#FC645F" href="https://developer.github.com/v3/">RESTful API</a>. Once this initial task is done, we'll refactor our code and provide a form in which the user can enter in a username of anyone on Github and display their profile information through a template. 
...
Now, our URLs may be accessed as `http://127.0.0.1:8000/app/<view url>`. Next, we'll wire up our base url within `app/urls.py`:
...
#### Capturing POST parameters

As mentioned, performing a POST request will send parameters that can be accessed programmatically. Let's modify our `profile` method within `views.py` to access the `user` value that was passed through our form submission:
